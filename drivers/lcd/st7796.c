...
#ifndef CONFIG_LCD_ST7796_SPIMODE
...
#  define CONFIG_LCD_ST7796_FREQUENCY 1000000
#endif
...
#if !defined(CONFIG_LCD_ST7796_XRES)
#  define CONFIG_LCD_ST7796_XRES 320
#endif

#if !defined(CONFIG_LCD_ST7796_YRES)
#  define CONFIG_LCD_ST7796_YRES 480
#endif

#define ST7796_LUT_SIZE    CONFIG_LCD_ST7796_YRES

#if defined(CONFIG_LCD_LANDSCAPE) || defined(CONFIG_LCD_RLANDSCAPE)
#  define ST7796_XRES       CONFIG_LCD_ST7796_YRES
#  define ST7796_YRES       CONFIG_LCD_ST7796_XRES
#  define ST7796_XOFFSET    CONFIG_LCD_ST7796_YOFFSET
#  define ST7796_YOFFSET    CONFIG_LCD_ST7796_XOFFSET
#else
#  define ST7796_XRES       CONFIG_LCD_ST7796_XRES
#  define ST7796_YRES       CONFIG_LCD_ST7796_YRES
#  define ST7796_XOFFSET    CONFIG_LCD_ST7796_XOFFSET
#  define ST7796_YOFFSET    CONFIG_LCD_ST7796_YOFFSET
#endif
...
struct st7796_dev_s
{
  ...
  uint16_t runbuffer[ST7796_LUT_SIZE];
};
...
static void st7796_setorientation(FAR struct st7796_dev_s *dev);
static void st7796_display(FAR struct st7796_dev_s *dev, bool on);
static void st7796_setarea(FAR struct st7796_dev_s *dev,
                           uint16_t x0, uint16_t y0,
                           uint16_t x1, uint16_t y1);
static void st7796_bpp(FAR struct st7796_dev_s *dev, int bpp);
static void st7796_wrram(FAR struct st7796_dev_s *dev,
                         FAR const uint16_t *buff, size_t size);
#ifndef CONFIG_LCD_NOGETRUN
static void st7796_rdram(FAR struct st7796_dev_s *dev,
...
static int st7796_putrun(fb_coord_t row, fb_coord_t col,
                         FAR const uint8_t *buffer, size_t npixels);
static int st7796_putarea(fb_coord_t row_start, fb_coord_t row_end,
                          fb_coord_t col_start, fb_coord_t col_end,
                          FAR const uint8_t *buffer);
#ifndef CONFIG_LCD_NOGETRUN
static int st7796_getrun(fb_coord_t row, fb_coord_t col,
...
static void st7796_select(FAR struct spi_dev_s *spi, int bits)
{
  /* Select ST7796 chip (locking the SPI bus in case there are multiple
  ...
  /* Now make sure that the SPI bus is configured for the ST7796 (it
...
static void st7796_deselect(FAR struct spi_dev_s *spi)
{
  /* De-select ST7796 chip and relinquish the SPI bus. */
...
static inline void st7796_sendcmd(FAR struct st7796_dev_s *dev, uint8_t cmd)
{
  st7796_select(dev->spi, ST7796_BYTESPP * 8);
...
static void st7796_display(FAR struct st7796_dev_s *dev, bool on)
{
  st7796_sendcmd(dev, on ? ST7796_DISPON : ST7796_DISPOFF);
  st7796_sendcmd(dev, ST7796_INVON);
}

/****************************************************************************
 * Name: st7796_setorientation
 *
 * Description:
...
#if !defined(CONFIG_LCD_PORTRAIT)
  st7796_sendcmd(dev, ST7796_MADCTL);
                           ...
                           uint16_t x1, uint16_t y1)
{
  ...
  SPI_SEND(dev->spi, y0 + ST7796_YOFFSET);
  SPI_SEND(dev->spi, y1 + ST7796_YOFFSET);
  st7796_deselect(dev->spi);
  ...
  st7796_select(dev->spi, 16);
  SPI_SEND(dev->spi, x0 + ST7796_XOFFSET);
  SPI_SEND(dev->spi, x1 + ST7796_XOFFSET);
  st7796_deselect(dev->spi);
}
  ...
  if (dev->bpp != bpp)
    {
      ...
      depth = bpp >> 2 | 1;
      st7796_sendcmd(dev, ST7796_COLMOD);
      st7796_select(dev->spi, 8);
...
static void st7796_wrram(FAR struct st7796_dev_s *dev,
                         FAR const uint16_t *buff, size_t size)
{
  st7796_sendcmd(dev, ST7796_RAMWR);
...
static void st7796_rdram(FAR struct st7796_dev_s *dev,
                         FAR uint16_t *buff, size_t size)
{
  st7796_sendcmd(dev, ST7796_RAMRD);
...
static void st7796_fill(FAR struct st7796_dev_s *dev, uint16_t color)
{
  ...
  st7796_setarea(dev, 0, 0, ST7796_XRES - 1, ST7796_YRES - 1);

  st7796_sendcmd(dev, ST7796_RAMWR);
  st7796_select(dev->spi, ST7796_BYTESPP *8);

  for (i = 0; i < ST7796_XRES * ST7796_YRES; i++)
    {
...
#ifndef CONFIG_LCD_NOGETRUN
static int st7796_getrun(fb_coord_t row, fb_coord_t col, FAR uint8_t *buffer,
                         size_t npixels)
{
  FAR struct st7796_dev_s *priv = &g_lcddev;
  ...
  st7796_setarea(priv, col, row, col + npixels - 1, row);
  st7796_rdram(priv, dest, npixels);
                               ...
                               FAR struct fb_videoinfo_s *vinfo)
{
  ...
  lcdinfo("fmt: %d xres: %d yres: %d nplanes: 1\n",
          ST7796_COLORFMT, ST7796_XRES, ST7796_YRES);

  vinfo->fmt     = ST7796_COLORFMT;    /* Color format: RGB16-565: RRRR RGGG GGGB BBBB */
  vinfo->xres    = ST7796_XRES;        /* Horizontal resolution in pixel columns */
  vinfo->yres    = ST7796_YRES;        /* Vertical resolution in pixel rows */
                               ...
                               FAR struct lcd_planeinfo_s *pinfo)
{
  ...
  pinfo->putrun = st7796_putrun;                  /* Put a run into LCD memory */
  pinfo->putarea = st7796_putarea;                /* Put an area into LCD */
#ifndef CONFIG_LCD_NOGETRUN
  pinfo->getrun = st7796_getrun;                  /* Get a run from LCD memory */
#endif
...
/****************************************************************************
 * Name:  st7796_initialize
 *
 * Description:
 *   Initialize the ST7796 video hardware.  The initial state of the
...
FAR struct lcd_dev_s *st7796_lcdinitialize(FAR struct spi_dev_s *spi)
{
  ...
  st7796_bpp(priv, ST7796_BPP);
  st7796_setorientation(priv);
  st7796_display(priv, true);
  st7796_fill(priv, 0xffff);
