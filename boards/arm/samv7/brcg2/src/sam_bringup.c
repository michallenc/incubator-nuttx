/****************************************************************************
 * boards/arm/samv7/brcg2/src/sam_bringup.c
 *
...
# endif
  ...
  ret = sam_w25qxxxjv_initialize(0);
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: sam_w25qxxxjv_initialize failed: %d.\n", ret);
    }
#endif
  ...
  snprintf(blockdev, sizeof(blockdev), "/dev/mtdblock%d", PROGMEM_MTD_MINOR);
  snprintf(chardev, sizeof(chardev), "/dev/mtd%d", PROGMEM_MTD_MINOR);
