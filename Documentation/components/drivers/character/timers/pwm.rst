...
It is necessary to open the device to get a file descriptor for further
operations.

.. c:function:: int open(const char *pathname, int flags);

  Opens a file. Files are required to be opened before any other file
  operations are performed on it.

  :param FAR const char * pathname: Relative path of the character device.
  :param int flags: Does not have any affect.
